...
pub fn main() !void {
    ...
    const cie_header = try CieHeader.parse(allocator, &stream);
    defer cie_header.deinit(allocator);
    var init_instructions = std.ArrayList(Instruction).init(allocator);
    defer init_instructions.deinit();
    try decodeInstructions(&stream, cie_header_offset, cie_header, cie_header.sizeInFile(), &init_instructions);
    const first_row_template: CfiRow = blk: {
        var row: CfiRow = .{ .location = undefined, .cfa = undefined };
        try executeAllInstructionsForRow(init_instructions.items, &row);
        break :blk row;
    };

    var entries = std.ArrayList(Fde).init(allocator);
    defer entries.deinit();
    while (try stream.getPos() < try stream.getEndPos()) {
        const fde = try Fde.parse(allocator, &stream, cie_header, first_row_template);
        try entries.append(fde);
    }

    for (entries.items) |entry| {
        try printFde(std.io.getStdOut(), entry);
    }
}

fn decodeInstructions(
    source: anytype,
...
fn executeInstructions(instructions: []const Instruction, table: *std.ArrayList(CfiRow)) !void {
...
fn executeAllInstructionsForRow(instructions: []const Instruction, row: *CfiRow) !void {
...
const register_count = 16 + 1;
const CfiRow = struct {
    location: u64,
    cfa: CfaRule,
    registers: [register_count]RegisterRule = .{.undefined} ** register_count,

    const ra_index = 16;
};

const CfaRule = struct {
    register: u8,
    offset: u64,
...
const RegisterRule = union(RegisterRuleTag) {
    undefined,
    offset: i64,
...
fn printFde(stream: anytype, fde: Fde) !void {
...
fn printCfiRow(stream: anytype, row: CfiRow) !void {
